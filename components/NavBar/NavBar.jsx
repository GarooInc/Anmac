            ...
            <div className="mt-6 flow-root">
              <div className="-my-6 divide-y divide-gray-500/10">
                <div className="space-y-2 py-6">
                  {navigation.map((item) => (
                    <Link key={item.name} to={item.href} spy={true} smooth={true} duration={500} onClick={() => setMobileMenuOpen(false)} className="font-montserrat -mx-3 block rounded-lg px-3 py-2 text-base font-semibold leading-7 text-blue_anmac hover:bg-blue_anmac hover:text-white cursor-pointer">
                      {item.name}
                    </Link>
                  ))}
                </div>
